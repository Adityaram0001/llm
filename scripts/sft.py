#!/usr/bin/env python
"""CLI entrypoint for phase 8 supervised fine-tuning — one config, one run folder.

Usage:
    python scripts/sft.py --config configs/sft_s_dictionary.yaml
    python scripts/sft.py --config configs/sft_s_dictionary.yaml --device cpu   # smoke test
"""
...
from llmlab.train.sft_config import SFTConfig
from llmlab.train.sft_trainer import SFTTrainer
...
def main() -> None:
    ...
    parser.add_argument("--config", type=Path, required=True, help="path to a configs/sft_*.yaml")
    ...
    slug = args.config.stem.removeprefix("sft_").replace("_", "-")
    run_id = args.run_id or f"{datetime.date.today():%Y%m%d}_p{cfg.phase}_sft-{slug}"
    ...
    trainer = SFTTrainer(cfg, run_dir)
    status = trainer.fit()
    print(f"\n{status}: {trainer.step}/{trainer.total_steps} steps, best SFT val_loss {trainer.best_val_loss:.4f}")
