...
| RW-3 | One-time cloud accounts setup. **Done:** GitHub remote, R2 bucket + rclone (D-026), Docker Desktop installed locally, $10 gpuhub credit purchased, provider decision (D-027: gpuhub, native image-snapshot workflow, RunPod kept documented-but-unbuilt). **Cloud pipeline validated live end-to-end 2026-07-12 (D-029)**: RTX 4080 Super dry-run instance (D-028), `scripts/cloud/gpuhub_setup.sh` ran clean over SSH, real training smoke test passed (99,554 tok/s), checkpoint round-tripped CUDA→Mac-MPS. Image saved as **"genesis"** — contains OS+deps+conda env+our SSH key+`.env` (system disk only; repo/data live on the data disk, NOT in the image — user confirmed the `.env`-in-image finding is an acceptable risk, no token rotation needed). **GPU capacity fully measured 2026-07-12, both tiers compared (D-030, D-031, D-032)**: `find_batch_size.py` run across all 3 tiers × all 3 seq_lens on both an RTX 4080 ($0.25/hr) and a real RTX 5090 ($0.46/hr). **Conclusion: default to RTX 5090 for all real runs — it's ~3x faster AND cheaper per completed run** (L-tier hero run: $3.43/13.7hr on 4080 vs $2.11/4.6hr on 5090), not a speed/cost tradeoff. 4080 tier now treated as a near-free dry-run/debugging sandbox only. Full reasoning + the "4080's throughput regresses past its sweet spot, 5090's mostly doesn't" hardware-difference finding: `docs/learnings/20260712_gpuhub-rtx4080-capacity.md`. **Before any real run**, set `configs/train_s_*.yaml`'s `micro_batch` to the GPU-specific sweet spot (table in `docs/CLOUD_GPUHUB.md` §10) — the Mac-tuned `micro_batch=16` default is suboptimal on both cloud GPUs.
